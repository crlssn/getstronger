...
var (
	exerciseAllColumns            = []string{"id", "user_id", "title", "sub_title", "created_at"}
	exerciseColumnsWithoutDefault = []string{"user_id", "title"}
	exerciseColumnsWithDefault    = []string{"id", "sub_title", "created_at"}
...
func (exerciseL) LoadRoutines(ctx context.Context, e boil.ContextExecutor, singular bool, maybeExercise interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"getstronger\".\"routines\".\"id\", \"getstronger\".\"routines\".\"user_id\", \"getstronger\".\"routines\".\"title\", \"getstronger\".\"routines\".\"created_at\", \"a\".\"exercise_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.UserID, &one.Title, &one.CreatedAt, &localJoinCol)
		if err != nil {
