...
func (o *Routine) GetExercises() ExerciseSlice {
	if o == nil {
		return nil
	}
...
func (o *Routine) GetWorkouts() WorkoutSlice {
	if o == nil {
		return nil
	}
...
func (r *routineR) GetWorkouts() WorkoutSlice {
	if r == nil {
		return nil
	}
...
func (o *Routine) Exercises(mods ...qm.QueryMod) exerciseQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"getstronger\".\"exercises_routines\" on \"getstronger\".\"exercises\".\"id\" = \"getstronger\".\"exercises_routines\".\"exercise_id\""),
		qm.Where("\"getstronger\".\"exercises_routines\".\"routine_id\"=?", o.ID),
	)
...
func (o *Routine) Workouts(mods ...qm.QueryMod) workoutQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"getstronger\".\"workouts\".\"routine_id\"=?", o.ID),
	)
...
func (routineL) LoadExercises(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoutine interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"getstronger\".\"exercises\".\"id\", \"getstronger\".\"exercises\".\"user_id\", \"getstronger\".\"exercises\".\"title\", \"getstronger\".\"exercises\".\"created_at\", \"getstronger\".\"exercises\".\"deleted_at\", \"getstronger\".\"exercises\".\"tags\", \"getstronger\".\"exercises\".\"metrics\", \"getstronger\".\"exercises\".\"rest_seconds\", \"a\".\"routine_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.UserID, &one.Title, &one.CreatedAt, &one.DeletedAt, &one.Tags, &one.Metrics, &one.RestSeconds, &localJoinCol)
		if err != nil {
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (routineL) LoadWorkouts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoutine interface{}, mods queries.Applicator) error {
	var slice []*Routine
	var object *Routine
	...
	query := NewQuery(
		...
		qm.WhereIn(`getstronger.workouts.routine_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.RoutineID) {
...
func (o *Routine) AddExercises(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Exercise) error {
	...
	for _, rel := range related {
		query := "insert into \"getstronger\".\"exercises_routines\" (\"routine_id\", \"exercise_id\") values ($1, $2)"
...
func (o *Routine) SetExercises(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Exercise) error {
	query := "delete from \"getstronger\".\"exercises_routines\" where \"routine_id\" = $1"
...
func (o *Routine) RemoveExercises(ctx context.Context, exec boil.ContextExecutor, related ...*Exercise) error {
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"getstronger\".\"exercises_routines\" where \"routine_id\" = $1 and \"exercise_id\" in (%s)",
...
// Sets related.R.Routine appropriately.
func (o *Routine) AddWorkouts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Workout) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.RoutineID, o.ID)
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
...
// SetWorkouts removes all previously related items of the
// routine replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Routine's Workouts accordingly.
// Replaces o.R.Workouts with related.
// Sets related.R.Routine's Workouts accordingly.
func (o *Routine) SetWorkouts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Workout) error {
	query := "update \"getstronger\".\"workouts\" set \"routine_id\" = null where \"routine_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	if o.R != nil {
		for _, rel := range o.R.Workouts {
			queries.SetScanner(&rel.RoutineID, nil)
			if rel.R == nil {
				continue
			}
	...
	return o.AddWorkouts(ctx, exec, insert, related...)
}

// RemoveWorkouts relationships from objects passed in.
// Removes related items from R.Workouts (uses pointer comparison, removal does not keep order)
// Sets related.R.Routine.
func (o *Routine) RemoveWorkouts(ctx context.Context, exec boil.ContextExecutor, related ...*Workout) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("routine_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.Workouts {
			...
			ln := len(o.R.Workouts)
			if ln > 1 && i < ln-1 {
				o.R.Workouts[i] = o.R.Workouts[ln-1]
			}
			o.R.Workouts = o.R.Workouts[:ln-1]
			break
		}
	}
