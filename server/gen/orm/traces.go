...
var (
	traceAllColumns            = []string{"id", "request", "duration_ms", "status_code", "created_at"}
	traceColumnsWithoutDefault = []string{"request", "duration_ms", "status_code"}
...
func (o TraceSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"getstronger\".\"traces\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, tracePrimaryKeyColumns, len(o))
