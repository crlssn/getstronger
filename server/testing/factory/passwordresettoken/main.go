...
func main() {
	...
	email := flag.String("email", "", "the address to print the password reset token of")
	flag.Parse()
	...
	fmt.Println(auth.PasswordResetToken.GetOrZero().String()) //nolint:forbidigo // The token is this command's output.
}
