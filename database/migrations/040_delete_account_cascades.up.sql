...
-- against it intact.
--
-- Each constraint is replaced rather than added NOT VALID and validated after:
-- the migrate driver runs a whole file as one implicit transaction, so the
-- ADD's ACCESS EXCLUSIVE lock would be held across the VALIDATE scan anyway.
-- Splitting that into a second migration is the only way it would pay, and the
-- rows it re-checks are already covered by the constraints being replaced.
...
ALTER TABLE public.followers
    ...
    DROP CONSTRAINT IF EXISTS followers_followee_id_fkey,
...
ALTER TABLE public.workout_comments
    DROP CONSTRAINT IF EXISTS workout_comments_user_id_fkey,
    ADD CONSTRAINT workout_comments_user_id_fkey
        FOREIGN KEY (user_id) REFERENCES public.users (id) ON DELETE CASCADE,
    DROP CONSTRAINT IF EXISTS workout_comments_workout_id_fkey,
...
ALTER TABLE public.plan_routines
    DROP CONSTRAINT IF EXISTS plan_routines_routine_id_fkey,
