...
<template>
  ...
  <ul role="list" class="divide-y divide-gray-100 overflow-hidden bg-white shadow-sm ring-1 ring-gray-900/5 rounded-xl">
    <li v-for="exercise in exercises" :key="exercise.id">
      <RouterLink :to="`/exercises/${exercise.id}/edit`" class="flex justify-between items-center gap-x-6 px-4 py-5 hover:bg-gray-50 sm:px-6m text-sm/6 text-gray-900">
        {{ exercise.name }}
