...
func (r *routineR) GetRoutineExercisesSortOrders() RoutineExercisesSortOrderSlice {
	if r == nil {
		return nil
	}
...
func (o *Routine) RoutineExercisesSortOrders(mods ...qm.QueryMod) routineExercisesSortOrderQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"getstronger\".\"routine_exercises_sort_order\".\"routine_id\"=?", o.ID),
	)
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (routineL) LoadRoutineExercisesSortOrders(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoutine interface{}, mods queries.Applicator) error {
	var slice []*Routine
	var object *Routine
	...
	query := NewQuery(
		...
		qm.WhereIn(`getstronger.routine_exercises_sort_order.routine_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Routine) AddRoutineExercisesSortOrders(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*RoutineExercisesSortOrder) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.RoutineID = o.ID
