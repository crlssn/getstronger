...
<style scoped>
.loading-card, .empty-card { @apply mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm; }
.loading-card { @apply space-y-4; }
.loading-card span { @apply block h-4 animate-pulse rounded-full bg-slate-100; }
.loading-card span:nth-child(1) { @apply w-32; }
.loading-card span:nth-child(2) { @apply h-8 w-52; }
.empty-card h1 { @apply text-xl font-semibold text-slate-950; }
.empty-card p { @apply mt-2 text-sm text-slate-500; }
.empty-card a { @apply mt-4 inline-flex min-h-11 items-center rounded-xl bg-indigo-600 px-4 text-sm font-semibold text-white; }
</style>
