...
export const measurementDefinitions = [
  { metric: ExerciseMetric.WEIGHT, field: 'weight', label: 'kg', inputmode: 'decimal' },
...
export const formatExerciseSet = (set: Partial<Set>, exercise?: Pick<Exercise, 'metrics'>) =>
  measurementsForExercise(exercise)
    .map(({ field }) => {
      ...
      switch (field) {
        case 'weight': return `${number(value)} kg`
        case 'reps': return `${number(value)} reps`
        case 'distance': return `${number(value)} km`
        case 'durationSeconds': return formatMeasurementDuration(value)
      }
    })
    .join(' · ')
