...
export const trackedWeeks = 8

export interface StreakWeek {
  /** How far back this tick sits; 0 is the week in progress. */
  weeksAgo: number
  complete: boolean
  current: boolean
  workoutCount: number
...
/**
 * The eight ticks of the streak track, oldest first.
 *
...
export const streakWeeks = (
  ...
  Array.from({ length: trackedWeeks }, (_, index) => {
    ...
    return {
      ...
      current: weeksAgo === 0,
      workoutCount: weekWorkoutCounts[`${week.weekYear}-${week.weekNumber}`] ?? 0,
    }
  })
