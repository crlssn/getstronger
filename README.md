...
- `assetlinks.json` needs the SHA-256 fingerprint of the certificate the app is *served* with. Under Play App Signing that is Google's app signing key, not the upload key the bundle leaves CI with — see the Releasing section.
...
1. **Accounts**: an Apple Developer Program membership and a Google Play developer account. The bundle id `studio.getstronger.app` is registered with Apple under team `SABQ6FPUMG` and can no longer change; register the same id with Google Play.
2. **iOS credentials**: `ASC_PRIVATE_KEY` — the `.p8` contents — as a repository secret; it authenticates the TestFlight upload. `APPLE_TEAM_ID`, `ASC_KEY_ID` and `ASC_ISSUER_ID` go in as repository variables: they name the key rather than granting anything, the Team ID is already public in `apple-app-site-association`, and masking them only hides which identifier a signing failure objected to.
3. **iOS signing**: the archive signs manually against a stored certificate. Cloud signing (`-allowProvisioningUpdates`) is the obvious alternative and the wrong one — a runner's keychain starts empty, so it mints a certificate per run until the account hits Apple's limit and every release fails on `Choose a certificate to revoke`. In the [developer portal](https://developer.apple.com/account/resources/certificates/list), create an Apple Distribution certificate and an App Store provisioning profile for `studio.getstronger.app`, export the certificate from Keychain Access as a `.p12`, and add three repository secrets:
   - `IOS_CERTIFICATE_BASE64` — `base64 -i certificate.p12 | pbcopy`
   - `IOS_CERTIFICATE_PASSWORD` — the password chosen during that export
   - `IOS_PROVISIONING_PROFILE_BASE64` — `base64 -i profile.mobileprovision | pbcopy`

   The workflow reads the profile's own name into `RELEASE_PROVISIONING_PROFILE`, so recreating the profile under a different name needs no code change. Only the Release configuration signs manually; `mise run app:ios` builds Debug and still signs automatically, so local development needs none of this. Certificate and profile both expire after a year.
4. **Android credentials**: the upload keystore as `ANDROID_KEYSTORE_BASE64`, its `ANDROID_KEYSTORE_PASSWORD`, `ANDROID_KEY_PASSWORD` if the key's password differs from the store's, and the Play service account JSON as `PLAY_SERVICE_ACCOUNT_JSON` — all repository secrets. `ANDROID_KEY_ALIAS` goes in as a repository variable for the same reason the iOS identifiers do, with one of its own: GitHub redacts a secret's value wherever it appears in a log, so an alias like `upload` would blank the word out of every step that prints it. Play rejects an API publish until a new app's first `.aab` has gone through the Console by hand, so run the workflow once with `play_upload` off and hand-upload the `android-release-aab` artifact it leaves behind.
5. **Deep-link identities**: `apple-app-site-association` carries the Team ID already; `assetlinks.json` still needs an Android fingerprint. Take it from Play Console → Test and release → App integrity → app signing key, not from the upload keystore: Play re-signs every bundle, so the upload key's fingerprint leaves verification silently failing forever. Redeploy the web app after filling it.
6. **Store listings**: screenshots (the `mise run screenshots` contact sheet is a good source), the privacy policy URL (`https://www.getstronger.studio/privacy`), the account deletion URL (`https://www.getstronger.studio/delete-account`), and the privacy declarations described below.
7. **A review account**: every screen sits behind a sign-in, so App Review and Play both need working credentials for the production API. Create a real account and hand it over in the review notes; do not point them at a seed persona, which only exists locally.
...
The run names both sets it compared, reports the pages that moved, records them in the manifest, and writes a highlighted image of each difference to `web/screenshots/<ref>/changes/`, alongside a `pages.tsv` naming every page it found and how it moved — added, removed, resized, or changed — and an `against` file naming the set the before column came from. A one-line change to the auth flow's `.eyebrow`, for example, reports login, signup, forgot password, reset password, and the verification notice — including the pages nobody thought to check. Pass a pattern to compare a subset, as `screenshots:page` does.
