...
1. **Accounts**: an Apple Developer Program membership and a Google Play developer account; register the bundle id `studio.getstronger.app` in both (or change it in `mobile/capacitor.config.ts` first — it is provisional until the first store submission).
...
4. **Deep-link identities**: `apple-app-site-association` carries the Team ID already; `assetlinks.json` still needs the SHA-256 fingerprint of the Android upload key (`keytool -list -v -keystore …`). Redeploy the web app after filling it.
